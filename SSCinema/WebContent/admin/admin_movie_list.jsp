			...
			<c:if test="${!empty m_list }">
				<ul class="pagination">
					<c:if test="${page > blockNum }">
						<li class="page-item"><a class="page-link" href="admin_movie_list.do?page=1">◀◀</a></li>
						<li class="page-item"><a class="page-link" href="admin_movie_list.do?page=${page - 1 }">◀</a></li>
					</c:if>
				
					<c:forEach begin="${startBlock }" end="${endBlock }" var="i">
						<c:if test="${i == page }">
							<li class="active"><a class="page-link" href="<%=request.getContextPath() %>/admin_movie_list.do?page=${i }">${i }</a></li>
						</c:if>
						<c:if test="${i != page }">
							<li class="page-item"><a class="page-link" href="<%=request.getContextPath() %>/admin_movie_list.do?page=${i }">${i }</a></li>
						</c:if>
					</c:forEach>
					
					<c:if test="${page != allPage }">
						<li class="page-item"><a class="page-link" href="admin_movie_list.do?page=${page + 1 }">▶</a></li>
						<li class="page-item"><a class="page-link" href="admin_movie_list.do?page=${allPage }">▶▶</a></li>
					</c:if>
				</ul>
			</c:if>
		</div>
